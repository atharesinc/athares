...
aws s3 cp s3://www.athar.es/expo-service-worker.js s3://www.athar.es/expo-service-worker.js --metadata-directive REPLACE --cache-control max-age=0,no-cache,no-store,must-revalidate --content-type application/javascript --acl public-read
...
aws s3 cp s3://www.athar.es/manifest.json s3://www.athar.es/manifest.json --metadata-directive REPLACE --cache-control max-age=0,no-cache,no-store,must-revalidate --content-type application/json --acl public-read
